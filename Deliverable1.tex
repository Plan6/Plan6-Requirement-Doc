...
values of each letter grade can be redefined by the teacher.
	...
	\item A teacher adds automated-evaluational content.
	\begin{enumerate}[{VP2}.1]
		\item Student Viewpoint
			\begin{enumerate}
...
\underline{course content} which is not made visible by the owner of that 
course.
			\end{enumerate}
		\item Teacher Viewpoint
			\begin{enumerate}
				\item A teacher must be able to permanently remove \underline{course 
content}
...
average for all courses is not displayed.
	...
	\item A student enrolled in a course submits a \underline{student assignment}.
	\begin{enumerate}[{VP2}.1]
		\item Student Viewpoint
			\begin{enumerate}
...
policies is uploaded to the system.
	...
	\item The school administration deletes a course.
	\begin{enumerate}[{VP1}.1]
		\item Student Viewpoint
			\begin{enumerate}
